...
#
#    Suppose we chose a set of deletion indices D such that
#    after deletions, each remaining column in A is in
...
class Solution:
    def minDeletionSize(self, A):
        counter = 0
        for column in zip(*A):
            if tuple(sorted(column)) != column:
                counter += 1
        return counter


if __name__ == "__main__":
    testinput = ["cba", "daf", "ghi"]
    print(Solution.minDeletionSize(Solution, testinput))
